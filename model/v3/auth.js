...
async function addOverview() {
    try {
        var c=0
        const mv=await db.getMovieTvCollection()
        const mvs=await mv.find({})
        mvs.forEach(async (element) => {
            const {data}=await axios.get(`https://api.themoviedb.org/3/${element.type}/${element.id}?api_key=dfc43a605d906f9da6982495ad7bb34e`)
            let n=await mv.findByIdAndUpdate(element._id,{overview:data.overview},{new:true})
            if (n.overview) {
                c++
            }
            console.log(c);
...
model.verifyUser=async(id)=>{
    let userModel=await db.getUserCollection()
    let user=await userModel.findByIdAndUpdate(id,{verified:true},{new:true})
    if(user){
        return user
    }
    else{
...
model.updateUser=async(id,data)=>{
    let userModel=await db.getUserCollection()
...
model.addToWatchlist=async(data)=>{
    let userModel=await db.getUserCollection()
    let movietvModel=await db.getMovieTvCollection()
    let movietv=await movietvModel.find({id:data.id})
    if (!movietv.length) {
        let movietvRes=await movietvModel.insertMany([data])
        let user=await userModel.findByIdAndUpdate(data.user._id,{$push:{watchlist:{data:movietvRes[0]._id} }},{new:true}).populate("watchlist.data")
        if (user) {
        ...
        let user=await userModel.findByIdAndUpdate(data.user._id,{$push:{watchlist:{data:movietv[0]._id} }},{new:true}).populate("watchlist.data")
        if (user) {
            return user.watchlist
        } else {
            return null
...
model.removeFromWatchlist=async(data)=>{
    let userModel=await db.getUserCollection()
    let movietvModel=await db.getMovieTvCollection()
    let user=await userModel.findByIdAndUpdate(data.user._id,{$pull:{watchlist:{_id:data.watchlistId}}},{new:true}).populate("watchlist.data")
    if (user) {
        return user.watchlist
    } else {
        return null
